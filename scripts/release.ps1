...
5. Build and locally test the package, always naming the version - the script stamps the clock
   when -Version is absent: ./msix/build-msix.ps1 -SelfSign -Version <version>.0
6. Create and push tag v$version only after explicit release approval.
7. Verify the four assets release.yml generated: the zip, the
   *-windows-x64-setup.exe installer, and a SHA256 sidecar for each. Then run
