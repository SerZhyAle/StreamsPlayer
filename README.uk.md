...
- Перемикайтеся в режим сітки, який зберігається між запусками: видимі
  HTTP(S)-відеоканали захоплюються послідовно, а останні 64 кадри зберігаються
  локально.
...
- Видаляйте всі завантажені з каталогу потоки однією підтвердженою дією у
  «Налаштуваннях», щоб залишилися лише ваші записи `MANUAL` та `IMPORTED`;
  «Оновити каталог» завантажить їх знову, коли вони знадобляться.
...
- Зберігайте стан каталогу, ручні записи, закріплення, історію прослуховування та діагностичний журнал поточного сеансу `Current.log` у
...
буферизації. Для кадрів сітки також використовується LibVLC. Відтворення прямого
ефіру відновлюється після тимчасових мережевих збоїв і беззвучних зупинок за
обмеженою політикою повторів: показується окремий стан повторного підключення, а
коли спроби вичерпано - зрозумілий вибір «Повторити» або «Закрити».
ICY-метадані та розширені елементи керування плеєром залишаються наступними
етапами.
...
скористайтеся «Копіювати команду» або «Створити ярлик на робочому столі». Ці
