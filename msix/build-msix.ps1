...
[CmdletBinding()]
param(
    [string] $IdentityName = 'SerZhyAle.StreamsPlayer',
...
$publish = Join-Path $root 'src\StreamsPlayer.App\bin\Release\net10.0-windows\win-x64\publish'
if (-not (Test-Path (Join-Path $publish 'StreamsPlayer.exe'))) { throw "Published StreamsPlayer.exe not found in $publish." }
