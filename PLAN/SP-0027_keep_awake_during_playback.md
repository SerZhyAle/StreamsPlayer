...
**Status:** Partial - the preference and its persistence are verified; the idle-sleep effect itself still needs a human with an elevated shell. Exit: owner shortens the Windows idle-sleep timeout and confirms suppression while playing, normal sleep after stopping, and no orphaned request (`powercfg /requests`).
...
- expected: the option is present, localized and persists | actual: Settings -> Playback shows
  `Keep the computer awake during playback` / `Не давать компьютеру засыпать во время воспроизведения`,
  default on, and the saved value round-tripped through a restart.
- Not covered here: the actual power effect. `powercfg /requests` needs an elevated prompt (this session has
  none) and observing idle sleep needs real elapsed idle time and a power-plan change, so AC 1-AC 5 remain a
  human check.
