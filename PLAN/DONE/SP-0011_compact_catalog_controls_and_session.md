...
- expected: query, filters, sort and list position restored after a full restart | actual: query `jazz`,
  media `Audio`, sort `Country`, min-bitrate `128` and the collapsed-section flag all restored; row count
  identical (29 of 3,244) and the saved anchor channel (`NPO Radio 2 Soul & Jazz`) realized inside the viewport.
- expected: a saved anchor that the current filter excludes opens the list at the top without error |
  actual: anchor patched to a channel outside the `jazz` query -> vertical scroll 0 %, no dialog, no log error.
- expected: one-line header and one-line filter row | actual: observed in `tmp/uia/shots/sp0011-restored.png`.
