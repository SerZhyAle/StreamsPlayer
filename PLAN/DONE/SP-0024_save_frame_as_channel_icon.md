...
- expected: the camera-glyph button sits on the player bottom panel | actual: `Save icon` / `Сохранить значок`
  present in every player window opened.
- expected: a legible toast fades over the video | actual: `Кадр сохранён как значок канала`
  (`tmp/uia/shots/sp0024-toast.png`).
- expected: the grid tile adopts that exact frame and keeps it across a restart | actual: tile matched the saved
  frame immediately (`sp0024-grid-tile.png`) and, with automatic thumbnails switched off, the same frame was
  still on the tile after a restart (`sp0024-tile-after-restart2.png`). With automatic thumbnails on, a later
  capture legitimately replaced it - Decision 1 (newest wins).
- expected: pressing it before any frame renders does nothing harmful | actual: clicked ~1 s after opening the
  window: no toast, no crash, playback continued.
