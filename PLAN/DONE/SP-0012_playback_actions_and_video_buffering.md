...
- expected: localized buffering progress before video, then cleared | actual: `Буферизация… 14%` observed on
  open, replaced by `Прямой эфир` once playing.
- expected: several video windows play independently | actual: three simultaneous player windows
  (`13 Festival`, `111 TV`, `10 TV`) each reporting `Прямой эфир`; closing one left the others playing.
- expected: shared three-dot menu with Open / Open fullscreen / Open in new window / shortcut / Edit / Pin |
  actual: exactly those items enumerated from the row menu.
