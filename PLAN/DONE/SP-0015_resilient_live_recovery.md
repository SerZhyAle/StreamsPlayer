...
- expected: buffering and reconnecting are visibly distinct, in both languages | actual: `Буферизация… 14%`
  (player) versus `Reconnecting to Dead Test Stream… (attempt 1..4 of 4)` / `Переподключение к … (попытка 4 из 4)`.
- expected: cancelling during a backoff wait never restarts the stream | actual: Stop audio pressed while
  `Переподключение… (попытка 1 из 4)` was showing -> `Ничего не воспроизводится`, still silent 25 s later,
  no failure dialog.
- expected: an exhausted budget ends in the terminal dialog | actual: `Stream unavailable` / `Поток недоступен`
  with Retry / Copy report / Delete|Hide / Keep; Retry re-attempted (`Connecting to Dead Test Stream…`).
- Still unobserved (needs a genuinely flaky live source and a real network interruption): recovery of a real
  flapping stream and the silent-freeze stall watchdog.
