...
- expected: Russian UI presents `Трансляции` everywhere | actual: main window `Трансляции`, settings
  `Настройки Трансляции`, player `Видео - Трансляции`, failure dialog `Поток недоступен`.
- expected: English UI presents `STREAMS Player` | actual: main `STREAMS Player`, settings
  `STREAMS Player settings`, player `STREAMS Player video`.
