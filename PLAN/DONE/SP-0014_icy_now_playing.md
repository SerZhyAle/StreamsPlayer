...
- expected: station line gains the ICY track text | actual: `Сейчас играет: <station> - Oden & Fatzo, THEOS,
  Noa Milee - Only You` and, on a second station, `… - Night - Mark Anthony Don''t Blame Me Edit`.
- expected: a station without ICY metadata shows station-only text and no error | actual: three such stations
  played with the station-only line and no dialog.
- expected: no stale track text after stop or a station switch | actual: switching stations replaced the whole
  line; stop returned `Ничего не воспроизводится`.
- expected: the line re-renders localized with its arguments preserved on a language switch | actual: proven on
  the multi-argument now-playing line (`Reconnecting to X… (attempt 4 of 4)` <-> `Переподключение к X…
  (попытка 4 из 4)`), which uses the same `SetNowPlaying`/`RefreshLocalizedStateText` path. Caveat: a
  *track* line could not be caught mid-toggle because the test stations blank `StreamTitle` between songs.
- expected: the observed track reaches the history entry | actual: `Corona - The Rhythm Of The Night…` and
  `Patchwork - Panorama` stored on their history rows.
