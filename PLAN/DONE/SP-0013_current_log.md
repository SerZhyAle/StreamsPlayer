...
Create a local `Current.log` for each StreamsPlayer session so development and support can inspect important application activity and failures after the fact.
...
- The current-session file is `%LOCALAPPDATA%\StreamsPlayer\Current.log`, alongside the existing local application state; an installed application directory is not assumed writable.
...
1. Each normal app launch creates a fresh `%LOCALAPPDATA%\StreamsPlayer\Current.log` and records startup and shutdown.
