...
1. Add `src/StreamPlayer.App/CurrentLog.cs`: create `%LOCALAPPDATA%\StreamPlayer\Current.log` anew, serialize UTC log lines with `Information` and `Error` entries, allow concurrent writer callbacks, and make all file failures no-ops. Static check: no write exceptions escape its public methods.
