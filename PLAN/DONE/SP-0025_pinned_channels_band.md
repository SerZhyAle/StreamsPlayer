...
- expected: pinning moves a channel into its own section and persists | actual: `Закреплённые` section appeared
  with the pinned card and `pinned=1` survived a restart.
- expected: a header click collapses and expands, and the state survives a restart | actual: pinned header ->
  `pinnedSectionCollapsed=True` -> click again `False`; `mainSectionCollapsed=True` was still true after a restart.
- expected: the pinned section stays anchored while the main list scrolls | actual: pinned header Y stayed 441
  while the list scrolled through three large increments.
- Note: the earlier blocker (WPF ignoring posted `WM_LBUTTON*`) was solved by injecting real `SendInput` clicks
  after an ALT-tap foreground unlock plus `HWND_TOPMOST` raise - see `tmp/uia/driver.ps1`.
