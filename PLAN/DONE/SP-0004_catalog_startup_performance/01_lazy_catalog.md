...
1. Update `src/StreamsPlayer.App/MainWindow.xaml.cs` so a channel row holds the atlas coordinates and lazily resolves its image when WPF reads the favicon binding; reuse a row only when its channel/atlas inputs still match.
