...
- expected: file import previews New/Duplicate/Invalid/Repeated and applies as IMPORTED | actual: fixture
  `tmp/sp0016/agent-sample.m3u` -> New 2, Already in your list 0, Invalid 1, Repeated 1; Apply added
  `Agent Test Alpha` and `Agent Test Beta` with `SourceOrigin.Imported`.
- expected: an HLS media manifest imports nothing and explains why | actual: "This is an HLS media manifest,
  not a channel list. No channels were imported." and the imported count stayed at 2.
- expected: export writes a UTF-8 M3U of user-owned rows in order | actual: `#EXTM3U` + both `#EXTINF` entries
  with their titles and URLs; status `Exported 2 channel(s).`
- expected: a credential-bearing URL warns before writing | actual: adding `http://user:pass@…?token=…` and
  exporting raised "One or more channels contain a password or token in the address…"; declining wrote nothing.
- Both the Open and Save As common dialogs were driven through UI Automation, so the pickers are covered.
