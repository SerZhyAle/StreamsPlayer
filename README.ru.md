...
- Удаляйте все загруженные из каталога трансляции одним подтверждённым действием
  в «Настройках», чтобы остались только ваши записи `MANUAL` и `IMPORTED`;
  «Обновить каталог» загрузит их снова, когда они снова понадобятся.
- Переключайте интерфейс между английским, русским и украинским языком через
  выбор языка в верхней панели; выбор восстанавливается при следующем запуске.
...
- Ставьте таймер сна для радио — 15/30/45/60 минут или время по часам — с
  обратным отсчётом рядом с кнопкой остановки; он переживает смену станции и
